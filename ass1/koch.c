...
void drawVertices(double startx, double starty, double endx, double endy, int level){
	...
	if(level>0){
		...
		midx=startx+(len/3.0)*cos(angle);
		midy=starty+(len/3.0)*sin(angle);
		printf("Point 2: { %f, %f }",midx,midy);
		glVertex3f(midx,midy,0.0);
		glVertex3f(midx,midy,0.0);
		midx=midx+(len/3.0)*cos((angle+60*M_PI/180));
		midy=midy+(len/3.0)*sin((angle+60*M_PI/180));
		printf("Point 3: { %f, %f }",midx,midy);
		glVertex3f(midx,midy,0.0);
		glVertex3f(midx,midy,0.0);
		midx=endx-(len/3.0)*cos(angle);
		midy=endy-(len/3.0)*sin(angle);
